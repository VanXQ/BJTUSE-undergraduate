...
void GameLogic::BuildMap(int gemspecies){
...
bool GameLogic::eliminate(bool noChange)
{
    if(!game_running)
        return false;
...
bool GameLogic::down()//每调用一次，所有能落下的块重新落下压实
{
    srand(GetTickCount());
    int isChanged = false;
    for(int i = 0; i < 8; i++)
...
void GameLogic::propsEliminate(int propsSpc, int x, int y)
{
    switch (propsSpc) {
    case 1: //3*3boom
        m_aMap[x][y] = 0;
...
void GameLogic::setgame_running(bool game_running){
    this->game_running=game_running;
}

bool GameLogic::swap(int a,int b,int m,int n){//传入坐标(a,b)和(m,n)
        if(game_running){
            if(a==m){//同行？

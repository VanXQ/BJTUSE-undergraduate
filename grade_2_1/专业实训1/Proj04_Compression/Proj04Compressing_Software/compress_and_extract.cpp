...
int compress(HuffNode Huffman_tree[], int n, long file_length, char *in_file_name, char *out_file_name) {
    ...
    char buffer[BYTE_SIZE] = "\0";//缓存

    unsigned char suffix[20];//后缀名
    unsigned int z = 0;//后缀名长度
    unsigned int strLen = strlen(in_file_name);//导入文件名长度
...
//    printf("Total length is %d Bytes",file_length);
